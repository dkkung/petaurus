...
- **`save()` `embedReport=`** - the human-readable statistics report table is now embedded in every output by default (`True`), so you can read it straight out of a saved file. `usermeta.dysonsphere.report` is a container keyed by section (currently `report.statistics`), leaving room for future renderings; in the SVG and PNG each section rides in its own readable channel (real newlines: `<metadata id="dysonsphere-report-statistics">` / `iTXt dysonsphere-report-statistics`). It never touches `description`. Set `embedReport=False` to keep only the structured block.
