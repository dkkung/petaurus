...
These facts are stored **only** as structured JSON — dysonsphere doesn't also write a prose "Generated by…" string (that would bloat the files and duplicate the block). For a human-readable statistics report, use `ds.add_statistics(..., report=True)` (prints to stdout) or `save="dir"` (writes a `.txt`).
